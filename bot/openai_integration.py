...
async def analyze_food_image(base64_image: str) -> dict:
    ...
    try:
        ...
        prompt = """
        Ты высококвалифицированный диетолог-эксперт с опытом прецизионного анализа пищи. Проанализируй фотографию еды, которую я прикрепляю, и предоставь максимально детальный и точный пищевой анализ с точностью до десятых долей.

        Твоя задача:
        1. Определи все продукты и блюда на фотографии, указав их примерный вес в граммах с точностью до 1 грамма.
        2. Рассчитай КБЖУ (калории, белки, жиры, углеводы) для всех блюд на фото с точностью до десятых долей (например, 837.5 ккал, 32.4 г белка).
        ...
        ВАЖНО: Калораж должен быть равен Б*4 + Ж*9 + У*4 если блюдо не содержит алкоголь. Значения белков жиров и углеводов должны быть достаточно точны для подсчета калорий.
